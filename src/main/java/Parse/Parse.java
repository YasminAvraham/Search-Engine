...
//                    }
                ...
                if(tkn.contains("-")){
                    ...
                    if(isNumber(tkn.substring(0,index))&& isNumber(tkn.substring(index+1,tkn.length()))){
                        //addToken(tkn.substring(0,index),numOfDocs);
                        //addToken(tkn.substring(index+1,tkn.length()),numOfDocs);
                        addToken(tkn,numOfDocs);
                        continue;
                    }
                    else{
//                        String [] splitLine=tkn.split("-");
//                        for(int k=0;k<splitLine.length;k++){
//                            if(!splitLine[k].equals(" ")){
//                                String s=splitLine[k];
//                                if(isStem && isWord(s)){
//                                    s=stemmer.stemString(s);
//                                }
//                                addToken(s,numOfDocs);
//                            }
//                        }
                        addToken(tkn,numOfDocs);
                        continue;
